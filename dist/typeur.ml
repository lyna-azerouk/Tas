...
let rec unify_types (t1,t2) =
    ...
    in
      ...
      | Fun_type (t1,t2), Fun_type (t3,t4) -> 
         unify_types (t1,t3); unify_types(t2,t4)
      | Ref_type t1, Ref_type t2 -> unify_types(t1,t2) (*failwith "No unification for ref type"*)
...
let rec is_non_expansive e = match e with
  Var _ -> true
| Unop (_,e) -> is_non_expansive e
| Binop (_,e1,e2) | Pair(e1,e2) | Cons (e1,e2) -> is_non_expansive e1 && is_non_expansive e2
| Cond (e1,e2,e3) ->  is_non_expansive e1 && is_non_expansive e2 && is_non_expansive e3
| App (e1,e2) -> false
| Abs (_, e) -> true 
| Letin(b,s,e1,e2) -> is_non_expansive e1 && is_non_expansive e2 
| Ref e -> false
;;

let   is_expansive e = not (is_non_expansive e)
;;
     ...
     match expri with
       ...
       | Letin (false,s,e1,e2) -> 
           let t1 = type_rec e1 in
             let new_env = (if is_expansive e1 then [(s,Forall([],t1))] else generalize_types gamma [ (s,t1) ])
             in
       ...
       | Ref e -> Ref_type (type_rec e) (*failwith "not yet implemented X"*)
   in
     type_rec;;
...
let ascii i = let s = Bytes.create 1 in s.[0] <- Char.chr  i;s ;;
let var_name n = 
  let rec name_of n = 
     let q,r = ((n / 26), (n mod 26))
     in 
        if q=0 then ascii (96+r)
        else Bytes.cat (name_of q) (ascii (96+r))
   in Bytes.cat (Bytes.of_string "'") (name_of n)
;;
  ...
  in 
    ...
    in 
      ...
      |  Var_type {contents=(Unknown n)} -> 
           ...
           in print_string (Bytes.to_string name)

...
(* gÃ©nÃ©rateur de noms frais de variables de types *)
...
(* vÃ©rificateur d'occurence de variables *)  
...
(* zipper d'une liste d'Ã©quations *)
...
(* remplace unee variable par un type dans un zipper d'Ã©quations *)
...
(* trouve un type associÃ© Ã  une variable dans un zipper d'Ã©quation *)
...
(* rÃ©sout un systÃ¨me d'Ã©quations *) 
let rec unification (e : equa_zip) (but : string) : ptype = 
  match e with 
    (* on a passÃ© toutes les Ã©quations : succes *)
    (_, []) -> (try trouve_but (rembobine e) but with VarPasTrouve -> raise (Echec_unif "but pas trouvÃ©"))
  ...
  | (e1, (Var v1, Var v2)::e2) ->  unification (substitue_type_zip (rembobine (e1,e2)) v2 (Var v1)) but
    (* une variable Ã  gauche : vÃ©rification d'occurence puis remplacement *)
  | (e1, (Var v1, t2)::e2) ->  if appartient_type v1 t2 then raise (Echec_unif ("occurence de "^ v1 ^" dans "^(print_type t2))) else  unification (substitue_type_zip (rembobine (e1,e2)) v1 t2) but
    (* une variable Ã  droite : vÃ©rification d'occurence puis remplacement *)
  ...
  | (e1, (Arr (t1,t2), Arr (t3, t4))::e2) -> unification (e1, (t1, t3)::(t2, t4)::e2) but 
    (* types fleche Ã  gauche pas Ã  droite : echec  *)
  | (e1, (Arr (_,_), t3)::e2) -> raise (Echec_unif ("type fleche non-unifiable avec "^(print_type t3)))     
    (* types fleche Ã  droite pas Ã  gauche : echec  *)
